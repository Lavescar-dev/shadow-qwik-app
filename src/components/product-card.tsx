...
import { addToCart, openDetail } from '../lib/actions';
  ...
  return (
    <div class="border border-term_dim p-5 hover:border-term_accent transition-all flex flex-col group bg-black">
      ...
      <div class="flex gap-2 mt-auto">
        <button type="button" onClick$={() => openDetail(app, product.id)} class="border border-term_dim py-2 flex-1 text-sm hover:text-term_accent hover:border-term_accent transition-all font-bold uppercase">[ Info ]</button>
        <button type="button" onClick$={() => addToCart(app, product)} class="border border-term_dim py-2 flex-1 text-sm bg-term_dim hover:bg-term_accent hover:text-term_bg hover:border-term_accent transition-all font-bold uppercase tracking-widest">[ Ekle ]</button>
      </div>
    </div>
  );
});
