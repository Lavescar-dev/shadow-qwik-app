...
export const DetailView = component$(() => {
  ...
  const product = useComputed$(() => {
    ...
    return Number.isFinite(id) ? inventory.productsById[id] ?? null : null;
  });

  const backHref = useComputed$(() => {
    const from = loc.url.searchParams.get('from');
    if (from === 'assembly') return '/assembly';
    if (from === 'electronic') return '/electronic';
    return '/';
  });
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
          ...
          {selected.image ? (
            ...
            <>
              <div
                ...
                style="background-image: repeating-linear-gradient(45deg, #00ffcc 25%, transparent 25%, transparent 75%, #00ffcc 75%, #00ffcc), repeating-linear-gradient(45deg, #00ffcc 25%, #050505 25%, #050505 75%, #00ffcc 75%, #00ffcc); background-position: 0 0, 10px 10px; background-size: 20px 20px;"
              ></div>
        ...
        <div class="flex flex-col">
          <div class="flex justify-between items-start mb-2">
            <span class="text-xs bg-term_dim text-gray-300 px-2 py-1 rounded-sm">{selected.tag}</span>
            <span class="text-xs text-gray-500 font-mono">PID: 0x{selected.id.toString(16).padStart(4, '0').toUpperCase()}</span>
          </div>
          ...
          <div class="mb-8 border border-term_dim p-5 bg-black">
            ...
            <ul class="text-sm text-gray-400 space-y-3 font-mono">
              {Object.entries(selected.specs).map(([label, value]) => (
                <li key={label} class="flex border-b border-term_dim/50 pb-2">
          ...
          <button
            type="button"
            onClick$={() => addToCart(app, selected)}
            class="mt-auto border-2 border-term_accent bg-term_accent/10 py-4 w-full text-term_accent hover:bg-term_accent hover:text-term_bg transition-all font-bold uppercase tracking-widest text-lg shadow-[0_0_15px_rgba(0,255,204,0.1)] hover:shadow-[0_0_20px_rgba(0,255,204,0.3)]"
          >
            [ Belleğe Pushla (Add to Cart) ]
          </button>
        </div>
      </div>
          ...
          {selected.comments.length ? (
            selected.comments.map((comment, index) => (
              <div
                ...
                class="border border-term_dim p-5 bg-[#050505] hover:border-term_dim/80 transition-colors"
              >
                <div class="flex justify-between items-start mb-3">
                  <span class="font-bold text-term_accent font-mono text-sm">@{comment.user}</span>
                  <span class="text-term_accent text-xs tracking-widest">{renderStars(comment.rate)}</span>
                </div>
                <p class="text-gray-400 text-sm leading-relaxed">{comment.text}</p>
              </div>
            ))
          ) : (
            <div class="text-gray-500 text-sm font-mono">No feedback logs found. Be the first to compile.</div>
          )}
        </div>
      </div>
    </div>
